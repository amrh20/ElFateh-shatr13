...
export class ProductCardComponent {
  ...
  addToCart(): void {
    if (!this.product) {
      this.notificationService.error('خطأ', 'المنتج غير متوفر');
      return;
    }
  ...
  getProductImage(): string {
    if (!this.product) return 'assets/images/placeholder.png';
    
    // Check if images array exists and has items
    if (this.product.images && Array.isArray(this.product.images) && this.product.images.length > 0) {
      return this.getCleanImageUrl(this.product.images[0]);
    }
    
    // Fallback to single image property
    if (this.product.image) {
      return this.getCleanImageUrl(this.product.image);
    }
  ...
  getDiscountAmount(): number {
    ...
    if (this.product.priceAfterDiscount && this.product.priceAfterDiscount > 0) {
      return (this.product.price || 0) - this.product.priceAfterDiscount;
    }
